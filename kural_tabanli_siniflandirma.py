...
agg_df = new_df.sort_values(ascending=False)
...
agg_df["customers_level_based"] = [f"{country}_{source}_{sex}_{age_group}" for country, source, sex, age_group in zip(agg_df["COUNTRY"], agg_df["SOURCE"], agg_df["SEX"], agg_df["AGE_group"])]
level_based = agg_df.groupby("customers_level_based").agg({"PRICE": "mean"}).reset_index()
level_based.head()
...
def predict(x):
    ...
    new_customer = agg_df[agg_df["customers_level_based"] == x]

    segment = new_customer["SEGMENT"].mode()[0]

    price = new_customer["PRICE"].mean()

    print(f"Yeni müşterinin tahmini segmenti = {segment}, Tahmini harcaması = {price}")
    return x

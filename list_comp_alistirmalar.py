...
df.rename(columns={col: f"NUM_ {col.upper()}" for col in numeric_columns}, inplace=True)
...
df.rename(columns={col: f"{col.upper()}FLAG" if "no" not in col.lower() else col.upper() for col in df.columns}, inplace=True)
...
new_cols = [col for col in df.columns if col.lower() not in [x.lower() for x in og_list]]
new_df = df[new_cols]
# # Notlar:
